...
namespace GoLogs.Services.DeliveryOrder.Api.Queries.GetHistoriesByDOOrderNumber
{
    public class Handler : IRequestHandler<Request, IList<HistoriesModel>>
    {
        ...
        {
            var histories = await _context.Histories.AllAsync(new Query().Where(nameof(History.DOOrderNumber), request.DOOrderNumber).ForPage(request.Page, request.PageSize), cancellationToken);
