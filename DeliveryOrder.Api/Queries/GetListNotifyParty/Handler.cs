...
namespace GoLogs.Services.DeliveryOrder.Api.Queries.GetListNotifyParty
{
    ...
    public class Handler : IRequestHandler<Request, List<Response>>
    {
        ...
        public async Task<List<Response>> Handle(Request request, CancellationToken cancellationToken)
        {
            ...
            var notifyParties = await _context.NotifyParties.AllAsync(new Query().ForPage(request.Page, request.PageSize), cancellationToken);
            var responses = new List<Response>();
            foreach (var notifyparty in notifyParties)
            {
                responses.Add(new Response { Id = notifyparty.Id, CargoOwnerId = notifyparty.CargoOwnerId, NotifyAddress = notifyparty.NotifyAddress.Split(';').ToList() });
            }
